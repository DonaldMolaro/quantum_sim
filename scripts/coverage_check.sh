        ...
        case "$resolved" in
          ...
          *.cc)
            if [ "$exclude_shor" -eq 1 ] && [ "$resolved" = "$root/algorithms/shor_classical.cc" ]; then
              ...
              :
            elif [ "$exclude_demo" -eq 1 ] && [ "$resolved" = "$root/demos/qubo_demo.cc" ]; then
              :
            elif [ "$exclude_demo" -eq 1 ] && [ "$resolved" = "$root/demos/vqa_demo.cc" ]; then
              :
            elif [ "$exclude_demo" -eq 1 ] && [ "$resolved" = "$root/demos/qaoa_demo.cc" ]; then
              :
            elif [ "$exclude_demo" -eq 1 ] && [ "$resolved" = "$root/demos/vqe_demo.cc" ]; then
              :
            elif [ "$exclude_demo" -eq 1 ] && [ "$resolved" = "$root/demos/anneal_demo.cc" ]; then
              :
            else
              filtered="$filtered $f"
            fi
